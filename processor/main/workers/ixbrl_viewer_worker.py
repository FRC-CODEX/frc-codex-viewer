...
class IxbrlViewerWorker(Worker):
    ...
    def work(
        ...
        assert filingDownload.target_path is not None
        packages = list(taxonomy_package_urls)
        report_path = None
        for parent in filingDownload.target_path.parents:
            ...
            if report_path and zipfile.is_zipfile(parent):
                ...
                break
        result = self._generate_viewer(filingDownload.target_path, viewer_directory, packages)
        ...
        if isReportPackageExtension(filingDownload.download_path.name):
            # Save packages for constructing OIM versions of the packages.
            oim_package_dest = oim_path / filingDownload.download_path.name
            shutil.copy(filingDownload.download_path, oim_package_dest)
        return WorkerResult(
            job_message.filing_id,
            success=True,
            ...
            logs=result.logs,
            filename=filingDownload.download_path.name,
